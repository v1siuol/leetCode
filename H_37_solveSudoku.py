...
Success
Details 
Runtime: 896 ms, faster than 13.26% of Python3 online submissions for Sudoku Solver.
Memory Usage: 14 MB, less than 5.54% of Python3 online submissions for Sudoku Solver.
"""
...
def is_valid(board, row, col, k):
    begin_row = 3*(row//3)
    begin_col = 3*(col//3)
    return not any((board[row][i] == k or board[i][col] == k or 
                    board[begin_row+i//3][begin_col+i%3] == k 
                    for i in range(9)))
